...
OUT = os.path.join(PROP, 'gallery-ru.html')
...
def main():
    import glob
    themes = []
    for f in sorted(glob.glob(os.path.join(PROP, 'ru', 'theme-*.json')),
                    key=lambda x: int(re.search(r'theme-(\d+)', x).group(1))):
        themes.append(json.load(open(f, encoding='utf-8')))
...
TEMPLATE = r'''<title>Modern Telegram in a 2013 Interface</title>
<style>
:root{
  --ground:#e9edf2;
  --panel:#ffffff;
